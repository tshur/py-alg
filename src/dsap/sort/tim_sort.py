...
def tim_sort[CT: SupportsRichComparison](iterable: Iterable[CT]) -> list[CT]:
    ...
    The algorithm finds natural, already sorted subsequences called "runs," then efficiently merges them together. 
    Insertion sort is used to sort small runs for optimal performance. Winthin 2 items for our sample case.(In real world: 32 or 64)
    ...
    Complexity:
        Time: O(nlogn), worst case when we have no existing ascending trend in the input array for optimization.
    ...
    def merge(array: list[CT], l: int, m: int, r: int) -> None:
        """
        ...
        len1, len2 = m - l + 1, r - m
        left = array[l : l + len1]
        right = array[m + 1 : m + 1 + len2]

        i = j = 0
        k = l
        #Merge with galloping finding big chuck to copy
        while i < len1 and j < len2:
            if left_wins >= MIN_GALLOP: #enter galloping mode code here
                left_wins = 0

            if right_wins >= MIN_GALLOP: #exit galloping mode code here
                right_wins = 0
